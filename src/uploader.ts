 ...
 */
function buildContentUrl(bookSlug: string, cantoNumber?: number, chapterNumber?: number, verseNumber?: string): string {
...
export async function processOne(pg: Client, item: any): Promise<void> {
  ...
  try {
    ...
    await pg.query("UPDATE imports.items SET content_hash = $1 WHERE id = $2", [contentHash, item.id]);
    ...
    // 3a. Ensure Book
    const book = await ensureBook({ vedabase_slug: parsed.book_code }, bookInfo);
    ...
    const introHtmlUa =
      parsed.language === "uk" && normalized.content_type === "chapter" ? sanitizeHtml(normalized.purport) : undefined;
    const introHtmlEn =
      parsed.language === "en" && normalized.content_type === "chapter" ? sanitizeHtml(normalized.purport) : undefined;
    ...
    // 4. MARK SUCCESS
    const contentUrl = buildContentUrl(book.slug, parsed.canto_number, parsed.chapter_number, parsed.verse_number);
