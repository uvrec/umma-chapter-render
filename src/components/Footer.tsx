export const Footer = () => {
  ...
  return <footer className="bg-card border-t border-border mt-auto" aria-label="Footer">
      <div className="container mx-auto px-4 py-6">
        <div className="flex flex-col items-center space-y-4">
          <ul className="flex items-center space-x-6">
            {socialLinks.map(link => <li key={link.name}>
                <a href={link.url} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" className="text-muted-foreground hover:text-primary transition-colors duration-200 inline-flex items-center justify-center p-2 -m-2" aria-label={link.name}>
                  {link.icon}
                </a>
              </li>)}
          </ul>
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Powered by Prabhupada Enhanced
            </p>
          </div>
        </div>
      </div>
