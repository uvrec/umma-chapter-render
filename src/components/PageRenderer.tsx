...
const BlockRenderer = ({ block, language }: { block: PageBlock; language: string }) => {
  switch (block.type) {
    case "text":
      const textContent = language === "ua" ? block.content.text_ua : block.content.text_en;
    ...
    case "heading": {
      ...
      const headingText = language === "ua" ? block.content.text_ua : block.content.text_en;
        ...
        <div className="verse-block space-y-4 my-6 p-6 bg-muted/30 rounded-lg">
          ...
          {display_blocks.commentary && (
            <div className="commentary">
              {language === "ua" && block.content.commentary_ua && <TiptapRenderer content={block.content.commentary_ua} />}
    ...
    case "image":
      const imageCaption = language === "ua" ? block.content.caption_ua : block.content.caption_en;
    ...
    case "audio":
      const audioCaption = language === "ua" ? block.content.caption_ua : block.content.caption_en;
      return block.content.url ? (
        <div className="my-6">
    ...
    case "video":
      const videoCaption = language === "ua" ? block.content.caption_ua : block.content.caption_en;
      return block.content.url ? (
        <div className="my-6">
