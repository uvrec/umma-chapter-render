...
export const BOOK_TEMPLATES: ImportTemplate[] = [
  ...
  },
  {
    ...
    translationPattern: /^\s*(?:ПЕРЕКЛАД|TRANSLATION|Переклад)/mi,
    commentaryPattern: /^\s*(?:ПОЯСНЕННЯ|PURPORT|Пояснення)/mi,
    // Flexible pattern: numbers, Roman numerals, or Ukrainian ordinals (uppercase and lowercase). Avoid \b (ASCII-only) — use lookahead instead.
    chapterPattern: /^\s*(?:ГЛАВА|РОЗДІЛ|CHAPTER|Глава|Розділ|Chapter|глава|розділ|chapter)\s+(\d{1,3}|[IVXLCDM]+|ПЕРША|ДРУГА|ТРЕТЯ|ЧЕТВЕРТА|П'ЯТА|ШОСТА|СЬОМА|ВОСЬМА|ДЕВ'ЯТА|ДЕСЯТА|ОДИНАДЦЯТА|ДВАНАДЦЯТА|ТРИНАДЦЯТА|ЧОТИРНАДЦЯТА|П'ЯТНАДЦЯТА|ШІСТНАДЦЯТА|СІМНАДЦЯТА|ВІСІМНАДЦЯТА|ДЕВ'ЯТНАДЦЯТА|ДВАДЦЯТА|перша|друга|третя|четверта|п'ята|шоста|сьома|восьма|дев'ята|десята|одинадцята|дванадцята|тринадцята|чотирнадцята|п'ятнадцята|шістнадцята|сімнадцята|вісімнадцята|дев'ятнадцята|двадцята)(?=\s|:|$)/mi,
  },
  {
    id: 'sri-isopanishad',
    name: 'Шрі Ішопанішад',
