/**
 * Parser for iskconpress/books repository
 * Downloads and parses English + Sanskrit from GitHub
 *
 * Format: DokuWiki-style markup
 * Source: https://github.com/iskconpress/books
 *
 * Usage: npx ts-node tools/parse-iskconpress.ts [canto_number]
 */
...
if (isNaN(CANTO) || CANTO < 1 || CANTO > 12) {
  console.error('Usage: npx ts-node tools/parse-iskconpress.ts [canto_number]');
  process.exit(1);
}
...
/**
 * Fetch a verse file from GitHub
 */
 ...
 */
function parseWikiVerse(content: string): ParsedVerse | null {
  const result: ParsedVerse = { verse_number: '' };

  try {
    // Extract Sanskrit (Devanagari) - in blockquote after ====== SB ... ======
    // Look for lines starting with > that contain Devanagari characters
...
async function main() {
  ...
  console.log(`Parsing SB Canto ${CANTO} from iskconpress/books`);
  console.log('='.repeat(60));

  // Check UK JSON exists
  ...
  console.log(`Parsed with EN: ${parsedWithEN}`);
