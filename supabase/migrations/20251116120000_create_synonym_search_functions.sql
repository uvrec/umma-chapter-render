...
BEGIN
  ...
  RETURN QUERY EXECUTE format($query$
    ...
    ORDER BY match_rank DESC, b.sort_order, c.chapter_number, v.verse_number_int
    LIMIT $5 OFFSET $6
  $query$,
...
-- Пошук слова "кришна" в українських синонімах (режим contains)
-- SELECT * FROM search_synonyms('кришна', 'ua', 'contains', 10, 0);

-- Пошук слова що починається з "кр" (режим starts_with)
-- SELECT * FROM search_synonyms('кр', 'ua', 'starts_with', 10, 0);

-- Точний пошук слова "krishna" в англійських синонімах
-- SELECT * FROM search_synonyms('krishna', 'en', 'exact', 10, 0);

-- Отримати список популярних термінів для автокомпліту
-- SELECT * FROM get_unique_synonym_terms('ua', NULL, 50);

-- Отримати терміни що починаються з "кр"
-- SELECT * FROM get_unique_synonym_terms('ua', 'кр', 20);
