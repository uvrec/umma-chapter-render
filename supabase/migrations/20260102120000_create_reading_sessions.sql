...
CREATE TABLE IF NOT EXISTS public.user_chapter_progress (
  ...
  UNIQUE(user_id, book_slug, COALESCE(canto_number, 0), chapter_number)
);
...
-- Also allow anonymous tracking with null user_id
...
RETURNS user_reading_sessions AS $$
DECLARE
...
BEGIN
  ...
  IF v_session.user_id IS NOT NULL THEN
    ...
    DO UPDATE SET
      total_reading_seconds = user_book_progress.total_reading_seconds + v_duration,
      total_sessions = user_book_progress.total_sessions + 1,
      last_read_at = now(),
      updated_at = now();
    ...
    ON CONFLICT (user_id, book_slug, COALESCE(canto_number, 0), chapter_number)
    DO UPDATE SET
      reading_seconds = user_chapter_progress.reading_seconds + v_duration,
      session_count = user_chapter_progress.session_count + 1,
      last_verse = COALESCE(p_end_verse, user_chapter_progress.last_verse),
      percent_read = GREATEST(user_chapter_progress.percent_read, p_percent_read),
      is_completed = CASE WHEN GREATEST(user_chapter_progress.percent_read, p_percent_read) >= 95 THEN true ELSE user_chapter_progress.is_completed END,
      completed_at = CASE WHEN GREATEST(user_chapter_progress.percent_read, p_percent_read) >= 95 AND user_chapter_progress.completed_at IS NULL THEN now() ELSE user_chapter_progress.completed_at END,
      last_read_at = now(),
      updated_at = now();

    -- Update daily stats
    INSERT INTO user_reading_daily_stats (user_id, stats_date, reading_seconds, sessions_count, verses_read)
    VALUES (v_session.user_id, CURRENT_DATE, v_duration, 1, p_verses_read)
    ON CONFLICT (user_id, stats_date)
    DO UPDATE SET
      reading_seconds = user_reading_daily_stats.reading_seconds + v_duration,
      sessions_count = user_reading_daily_stats.sessions_count + 1,
      verses_read = user_reading_daily_stats.verses_read + p_verses_read,
      updated_at = now();
  END IF;
...
GRANT EXECUTE ON FUNCTION end_reading_session TO authenticated;
GRANT EXECUTE ON FUNCTION end_reading_session TO anon;
...
BEGIN
  ...
  SELECT
    ...
    COUNT(CASE WHEN bp.overall_percent > 0 AND bp.overall_percent < 100 THEN 1 END)::INTEGER as books_in_progress,
    COUNT(CASE WHEN bp.completed_at IS NOT NULL THEN 1 END)::INTEGER as books_completed,
