...
BEGIN
  ...
  SELECT
    ...
    CASE
      WHEN language_code = 'uk' AND v.search_vector_uk IS NOT NULL AND ts_query != ''::tsquery THEN
        ts_rank_cd(v.search_vector_uk, ts_query, 32)::numeric
      WHEN language_code != 'uk' AND v.search_vector_en IS NOT NULL AND ts_query != ''::tsquery THEN
...
BEGIN
  ...
  (SELECT
    ...
    CASE
      WHEN language_code = 'uk' AND bp.search_vector_uk IS NOT NULL AND ts_query != ''::tsquery THEN
        ts_rank_cd(bp.search_vector_uk, ts_query)::numeric
      WHEN language_code != 'uk' AND bp.search_vector_en IS NOT NULL AND ts_query != ''::tsquery THEN
