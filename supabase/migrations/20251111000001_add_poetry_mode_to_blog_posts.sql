...
ALTER TABLE public.blog_posts
  ...
  ADD COLUMN IF NOT EXISTS synonyms_uk TEXT,
  ADD COLUMN IF NOT EXISTS synonyms_en TEXT,
  ADD COLUMN IF NOT EXISTS poetry_translation_uk TEXT,
...
ALTER TABLE public.blog_posts
  ...
  ADD COLUMN IF NOT EXISTS audio_synonyms_uk_url TEXT,
  ADD COLUMN IF NOT EXISTS audio_synonyms_en_url TEXT,
  ADD COLUMN IF NOT EXISTS audio_poetry_translation_uk_url TEXT,
  ADD COLUMN IF NOT EXISTS audio_poetry_translation_en_url TEXT,
  ADD COLUMN IF NOT EXISTS audio_commentary_uk_url TEXT,
...
COMMENT ON COLUMN public.blog_posts.synonyms_uk IS 'Ukrainian word-by-word translation in format "word1 — meaning1; word2 — meaning2" (for poetry mode)';
...
COMMENT ON COLUMN public.blog_posts.poetry_translation_uk IS 'Ukrainian literary translation of the verse (for poetry mode)';
...
COMMENT ON COLUMN public.blog_posts.audio_synonyms_uk_url IS 'Audio URL for Ukrainian word-by-word translation';
COMMENT ON COLUMN public.blog_posts.audio_synonyms_en_url IS 'Audio URL for English word-by-word translation';
COMMENT ON COLUMN public.blog_posts.audio_poetry_translation_uk_url IS 'Audio URL for Ukrainian literary translation';
COMMENT ON COLUMN public.blog_posts.audio_poetry_translation_en_url IS 'Audio URL for English literary translation';
COMMENT ON COLUMN public.blog_posts.audio_commentary_uk_url IS 'Audio URL for Ukrainian commentary/purport (content_uk in poetry mode)';
