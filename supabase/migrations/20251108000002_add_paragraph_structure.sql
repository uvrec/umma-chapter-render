...
ALTER TABLE public.verses
  ADD COLUMN IF NOT EXISTS commentary_uk_paragraphs JSONB,
  ADD COLUMN IF NOT EXISTS commentary_en_paragraphs JSONB,
  ADD COLUMN IF NOT EXISTS translation_uk_paragraphs JSONB,
  ADD COLUMN IF NOT EXISTS translation_en_paragraphs JSONB;

-- Add paragraph fields to chapters table for summaries
ALTER TABLE public.chapters
  ADD COLUMN IF NOT EXISTS summary_uk_paragraphs JSONB,
  ADD COLUMN IF NOT EXISTS summary_en_paragraphs JSONB;

-- Індекси для JSONB полів (для швидшого пошуку)
CREATE INDEX IF NOT EXISTS idx_verses_commentary_uk_paragraphs ON public.verses USING GIN (commentary_uk_paragraphs);
CREATE INDEX IF NOT EXISTS idx_verses_commentary_en_paragraphs ON public.verses USING GIN (commentary_en_paragraphs);
CREATE INDEX IF NOT EXISTS idx_chapters_summary_uk_paragraphs ON public.chapters USING GIN (summary_uk_paragraphs);
CREATE INDEX IF NOT EXISTS idx_chapters_summary_en_paragraphs ON public.chapters USING GIN (summary_en_paragraphs);

-- Add comments for documentation
COMMENT ON COLUMN public.verses.commentary_uk_paragraphs IS 'Пояснення структуроване по абзацах: [{"index": 0, "text": "..."}, ...]';
COMMENT ON COLUMN public.verses.commentary_en_paragraphs IS 'Commentary structured by paragraphs: [{"index": 0, "text": "..."}, ...]';
COMMENT ON COLUMN public.verses.translation_uk_paragraphs IS 'Переклад структурований по абзацах (якщо довгий): [{"index": 0, "text": "..."}, ...]';
COMMENT ON COLUMN public.verses.translation_en_paragraphs IS 'Translation structured by paragraphs (if long): [{"index": 0, "text": "..."}, ...]';
COMMENT ON COLUMN public.chapters.summary_uk_paragraphs IS 'Summary Ukrainian structured by paragraphs: [{"index": 0, "text": "..."}, ...]';
...
BEGIN
    ...
    commentary_uk_paragraphs = (
      ...
      FROM (
        SELECT
          ...
          unnest(string_to_array(commentary_uk, E'\n\n')) as paragraph
      ) p
  ...
  WHERE commentary_uk IS NOT NULL OR commentary_en IS NOT NULL;
