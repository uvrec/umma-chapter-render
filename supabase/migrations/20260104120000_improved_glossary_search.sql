-- ============================================================================
-- ПОКРАЩЕНИЙ ПОШУК ГЛОСАРІЮ з пагінацією та групуванням
-- ============================================================================

-- 1. Функція для server-side пошуку термінів глосарію з пагінацією
  ...
  RETURN QUERY EXECUTE format($query$
    WITH parsed_terms AS (
      ...
      CROSS JOIN LATERAL unnest(
        string_to_array(
          -- Очищуємо HTML теги перед парсингом
    ...
    filtered_terms AS (
      ...
      WHERE et.term != ''
        ...
        -- Фільтруємо за терміном якщо вказано
        AND ($1 IS NULL OR $1 = '' OR et.term ILIKE $1)
        -- Фільтруємо за значенням якщо вказано
...
BEGIN
  ...
  RETURN QUERY EXECUTE format($query$
    ...
    SELECT
      ...
      g.usage_count,
      g.books,
      (g.sample_meanings)[1:5] as sample_meanings, -- Обмежуємо до 5 значень
...
-- Коментарі
COMMENT ON FUNCTION public.search_glossary_terms_v2 IS
'Server-side пошук термінів глосарію з пагінацією. Підтримує пошук по терміну та значенню.';

COMMENT ON FUNCTION public.get_glossary_terms_grouped IS
'Отримання унікальних термінів глосарію з підрахунком використань та списком книг.';

COMMENT ON FUNCTION public.get_glossary_stats IS
'Статистика глосарію: загальна кількість термінів, унікальних, по книгах.';

COMMENT ON FUNCTION public.get_glossary_term_details IS
'Деталі конкретного терміну: всі використання в різних віршах з контекстом.';
